...
type LeadService interface {
	...
	Details(req dto.LeadDetailsRequest) (*dto.LeadDetailsResponse, error)
...
func (s *leadService) Create(req dto.LeadCreateRequest, userID int) (*dto.LeadDetailsResponse, error) {
	...
	// Handle initial follow-up if provided
	if req.FollowUpStatus != "" {
		...
		followUp := &model.LeadFollowUp{
			...
			Status:           req.FollowUpStatus,
			Remark:           req.FollowUpRemark,
			FollowUpDate:     now,
			FollowUpTime:     now.Format("15:04:05"),
			UUID:             uuid.New().String(),
			CreatedAt:        now,
			UpdatedAt:        now,
			UpdatedBy:        s.resolveUserName(userID),
		}

		// Ensure UUID is generated if util.GenerateUUID isn't available or if I should use uuid directly
		if followUp.UUID == "" {
			// fallback if util.GenerateUUID doesn't return - actually I should check imports.
			// util package is imported. I'll check if it has GenerateUUID or similar.
			// Repository uses "github.com/google/uuid". I should verify if service imports it.
			// Service imports: "camp-backend/dto", "camp-backend/model", "camp-backend/repository", "camp-backend/util", "errors", "fmt", "time".
			// It does NOT import google/uuid.
			// repo.AddFollowUp handles UUID generation if empty!
			// "if followUp.UUID == "" { followUp.UUID = uuid.New().String() }" in repo.
		}

		if req.NextFollowUpDate != "" {
			parsedDate, err := time.Parse("2006-01-02", req.NextFollowUpDate)
			if err == nil {
				followUp.NextFollowUpDate = &parsedDate
				followUp.NextFollowUpTime = req.NextFollowUpTime
			}
		}

		_ = s.repo.AddFollowUp(followUp)
	}
...
func (s *leadService) Details(req dto.LeadDetailsRequest) (*dto.LeadDetailsResponse, error) {
	var lead *model.Lead
	var err error

	if req.ID > 0 {
		lead, err = s.repo.GetByID(req.ID)
	} else if req.LeadUUID != "" {
		lead, err = s.repo.GetByUUID(req.LeadUUID)
	} else {
		return nil, errors.New("invalid request: provide id or lead_uuid")
	}
